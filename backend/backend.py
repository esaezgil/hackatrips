...
HOTELS_COMBINED_SEARCH = "http://sandbox.hotelscombined.com/api/2.0/hotels?destination={user_destination}&checkin={ci}&checkout={co}&rooms={rooms}&apiKey={api_key}&sessionID={session_id}"
...
def get_custom_hotels(city, sensitivity):
    ...
    hotel_results = get_hotels_combined_results(place_id)
...
def get_hotels_combined_results(place_id, guests=2, rooms=1, checkin='2018-04-03', checkout='2018-04-06'):
    session_id = uuid.uuid4()
    headers = {'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_12_6) AppleWebKit/603.3.8 (KHTML, like Gecko)'}
    hotels_combined_search_url = HOTELS_COMBINED_SEARCH.format(user_destination=place_id, ci=checkin, co=checkout, rooms=rooms, api_key=HC_KEY, session_id=session_id)
    # hotels_results = requests.get(hotels_combined_search_url, headers=headers)
    # hotels_results_json = json.loads(hotels_results._content)
